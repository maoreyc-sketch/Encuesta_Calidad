...
def es_duplicado(email):
    if pd.io.common.file_exists('Consolidado_Encuestas_MEN.csv'):
        try:
            # ✅ FIX: on_bad_lines='skip' evita ParserError por filas malformadas
            df_hist = pd.read_csv(
                'Consolidado_Encuestas_MEN.csv',
                encoding='utf-8-sig',
                on_bad_lines='skip'
            )
...
if 'resp_enc' not in st.session_state:
    st.session_state.resp_enc = {}
if 'oblig' not in st.session_state:
                ...
                if not st.session_state.editando:
                    ...
                    if st.button("💾 Guardar Cambios"):
                        st.session_state.datos_temp = {
                            'NOMBRE': nom, 'DANE': dan, 'DEPTO': dep, 'MUNI': mun,
            ...
            with c_de:
                if st.session_state.paso < len(hojas) - 1:
                    if st.button("Siguiente Bloque ➡️"):
                        faltan = [
                            c for c in obligatorios_hoja_actual
                    ...
                    if st.button("💾 FINALIZAR Y ENVIAR ENCUESTA"):
                        faltan_totales = [
                            c for c in st.session_state.oblig
                            if not st.session_state.resp_enc.get(c)
                            or str(st.session_state.resp_enc.get(c)).strip() in ["", "Seleccionar..."]
                        ]
                        if faltan_totales:
                            ...
                            archivo_existe = pd.io.common.file_exists('Consolidado_Encuestas_MEN.csv')
                            # ✅ FIX: quoting=csv.QUOTE_ALL protege campos con comas,
                            #         saltos de línea o comillas que corrompían el CSV
...
with st.expander("🔐 Panel de Administración (Uso Interno)"):
    ...
    if pw == "AdminMEN2026":
        if pd.io.common.file_exists('Consolidado_Encuestas_MEN.csv'):
            # ✅ FIX: on_bad_lines='skip' evita que filas malformadas derriben la app
            df_v = pd.read_csv(
                'Consolidado_Encuestas_MEN.csv',
                encoding='utf-8-sig',
                on_bad_lines='skip'
            ...
            st.dataframe(df_v)
            st.download_button(
                "📥 Descargar Excel de Respuestas",
                data=df_v.to_csv(index=False).encode('utf-8-sig'),
                file_name='Consolidado_MEN.csv',
                mime='text/csv'
            )
        else:
